...
import { scheduleWorkOrder } from "@/lib/work-orders/service";
import { jsonApiErrorResponse, jsonFromZodError } from "@/lib/http";

const schema = z.object({ scheduledDate: z.string().min(1) });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const workOrder = await scheduleWorkOrder(id, input);
